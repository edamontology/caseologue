...
**A general quality control** for ontology is run on EDAM using the ROBOT `report command <http://robot.obolibrary.org/report>`_ from the robot suite. 
Robot report is a **command line tool** calling a **customizable list** of `SPARQL queries <http://robot.obolibrary.org/report_queries/>`_  that ensure **basic requirement** for all ontology fallowing the obo guidelines (e.g. presence of a liscence, more than one label in a entity) 
